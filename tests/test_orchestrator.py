...
class TestRunDirectoriesPipeline(unittest.TestCase):
    ...
    def test_returns_dict(self, mock_empty, mock_populate):
        conn = MagicMock()
        result = run_directories_pipeline(conn, self.tmp, [self.sub1, self.sub2])
    ...
    @patch('pipeline.orchestrator.populate_directories_table')
    @patch('pipeline.orchestrator.is_directories_empty', return_value=True)
    def test_populate_called_for_each_folder(self, mock_empty, mock_populate):
        conn = MagicMock()
        run_directories_pipeline(conn, self.tmp, [self.sub1, self.sub2])
        # root + 2 subfolders = 3 calls
    ...
    def test_raises_if_table_not_empty(self, mock_empty):
        ...
        with self.assertRaises(Exception) as ctx:
            run_directories_pipeline(conn, self.tmp, [])
    ...
    def test_lookup_values_are_ints(self, mock_empty, mock_populate):
        conn = MagicMock()
        result = run_directories_pipeline(conn, self.tmp, [self.sub1])
    ...
    def test_parent_of_root_is_zero(self, mock_empty, mock_populate):
        conn = MagicMock()
        run_directories_pipeline(conn, self.tmp, [])
        first_call_record = mock_populate.call_args_list[0][0][1]
        self.assertEqual(first_call_record['Parent'], 0)
    ...
    def test_subfolder_parent_matches_parent_project_id(self, mock_empty, mock_populate):
        ...
        # Find the record written for sub1
        sub1_record = next(
            c[0][1] for c in mock_populate.call_args_list
            if c[0][1].get('ProjectID') == sub1_id
        )
        self.assertEqual(sub1_record['Parent'], root_id)
...
class TestRunFilesPipeline(unittest.TestCase):
    ...
    def test_calls_populate_for_each_file(self, mock_empty, mock_populate):
        conn = MagicMock()
        run_files_pipeline(conn, [self.file_a], self._make_lookup())
    ...
    def test_skips_unreadable_file_without_crashing(self, mock_empty, mock_populate):
        ...
        run_files_pipeline(conn, [ghost_file, self.file_a], self._make_lookup())
    ...
    def test_empty_file_list_no_inserts(self, mock_empty, mock_populate):
        conn = MagicMock()
        run_files_pipeline(conn, [], self._make_lookup())
...
class TestRunPipeline(unittest.TestCase):
    ...
    def test_directories_pipeline_result_passed_to_files_pipeline(
        ...
        conn = MagicMock()
        run_pipeline(conn, self.tmp)
        _, _, lookup_passed = mock_files.call_args[0]
